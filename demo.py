...
# Track which monitors are actually available
...
try:
    ...
    driver_cam = cv2.VideoCapture(-1) # Changed to -1 for automatic camera selection
...
try:
    ...
    if not os.path.exists(model_path):
        ...
        traffic_cam = cv2.VideoCapture(0) #  i can Changed to 0 for testing, adjust to 1 for actual traffic cam
        if not traffic_cam.isOpened():
            print("Warning: Could not open traffic camera (Index 1)! Traffic monitoring disabled.")
        else:
...
if DRIVE_MONITOR_ACTIVE:
    ...
    print("Skipping Driver Monitoring thread...")
...
if TRAFFIC_MONITOR_ACTIVE:
    ...
    print("Skipping Traffic Monitoring thread...")

# Keep system running if any thread is active
for t in threads:
    t.join()
